...
describe("buildUnits enable gates (wire-units.ts)", () => {
  ...
  it("still honors a set KUBECONFIG_PATH as the explicit file override", () => {
    ...
    expect(wiring.enabled).toBe(true);
    expect(wiring.tenantEnabled).toBe(true);
    expect(wiring.defs).toHaveLength(26); // 11 consumer (incl. purge, adopt-consumer, restart-workloads, set-size, backup/restore/migrate) + 15 tenant (those 12, plus check-tenants, tenant-apps-repo and tenant-apps-repo-purge)
  });
