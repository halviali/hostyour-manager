...
export function registerOnboardPrefillRoute(app: Hono<AppEnv>, deps: OnboardPrefillApiDeps): void {
  const { onboardingEnabled, github, platformGitHub, platformRepo, githubApp } = deps;
  app.post("/api/consumers/prefill", async (c) => {
    ...
    const view = await readOnboardPrefill({ github, ...(platformGitHub ? { platformGitHub } : {}), ...(platformRepo ? { platformRepo } : {}), ...(githubApp ? { githubApp } : {}) }, parsed.data, c.req.raw.signal);
