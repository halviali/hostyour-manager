...
/** The organisation identities: acme records its packages reader; any other organisation none. */
const organisations: OrganisationIdentityReader = (org) => (org === "acme" ? { packagesCredentialId: "cred_pkg", repoCredentialId: null } : null);
...
describe("refreshAppTokens", () => {
  it("rewrites the repo-pat of every unit whose build registration names a credential with the value opened NOW — the pat unit's PAT included — and deletes its three build Secrets behind the rewrite", async () => {
    ...
    expect(await refreshAppTokens({ store, organisations, registrations: reg, seeder, kube, logger, githubApp: new FakeGitHubApp() })).toEqual({ refreshed: ["acme-apps", "shop", "beta-apps"], failed: [] });
    ...
    await refreshAppTokens({ store, organisations, registrations: reg, seeder, kube, logger, githubApp: new FakeGitHubApp() });
  ...
  it("logs the unit whose write fails, with its name, deletes none of its Secrets, and refreshes the others", async () => {
    ...
    const { logger, errors } = fakeLogger();
    const kube = new FakeClusterReader();
    expect(await refreshAppTokens({ store, organisations, registrations: await registrations(), seeder, kube, logger, githubApp: new FakeGitHubApp() })).toEqual({ refreshed: ["shop", "beta-apps"], failed: ["acme-apps"] });
  ...
  it("logs the unit whose Secret deletion fails, with its name and its build namespace, counts it failed, and the tick goes on to the next unit without rejecting", async () => {
    ...
    expect(await refreshAppTokens({ store, organisations, registrations: await registrations(), seeder, kube, logger, githubApp: new FakeGitHubApp() })).toEqual({ refreshed: [], failed: ["acme-apps", "shop", "beta-apps"] });
  ...
  it("without a wired kube it rewrites Vault, counts the units refreshed, and logs the skipped deletion naming them", async () => {
    ...
    expect(await refreshAppTokens({ store, organisations, registrations: await registrations(), seeder, logger, githubApp: new FakeGitHubApp() })).toEqual({ refreshed: ["acme-apps", "shop", "beta-apps"], failed: [] });
    expect(written.map((w) => w.consumerName)).toEqual(["acme-apps", "shop", "beta-apps"]);
    expect(errors).toEqual([]);
  ...
  it("logs, writes nothing and does not reject when the registration tree cannot be read", async () => {
    ...
    expect(await refreshAppTokens({ store, organisations, registrations: broken, seeder, kube, logger, githubApp: new FakeGitHubApp() })).toEqual({ refreshed: [], failed: [] });
    expect(written).toEqual([]);
  ...
  it("does nothing, and says nothing, on an installation without a credentialed build unit", async () => {
    ...
    expect(await refreshAppTokens({ store, organisations, registrations: reg, seeder, kube, logger, githubApp: new FakeGitHubApp() })).toEqual({ refreshed: [], failed: [] });
    expect(written).toEqual([]);
    expect(kube.secretWrites).toEqual([]);
    expect(errors).toEqual([]);
    expect(warns).toEqual([]);
