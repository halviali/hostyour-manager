...
// github-app credential has its build repo-pat rewritten with the value the store opens to now; a
// pat unit is left alone; one unit's failure is logged and the rest go on; nothing rejects.
...
import { refreshAppTokens, refreshUnitRepoPat } from "./app-token-refresh.ts";
...
function fakeLogger(): { logger: Logger; errors: string[]; infos: string[] } {
  const errors: string[] = [];
  const infos: string[] = [];
  const logger = {
...
describe("refreshAppTokens", () => {
  it("rewrites the repo-pat of every unit whose build registration names a github-app credential with the value opened NOW, and skips the pat unit", async () => {
    const minted = { value: "ghs_minted_at_tick_1" };
    const { store, opened } = fakeStore(minted);
    const { seeder, written } = fakeSeeder();
    const { logger, errors, infos } = fakeLogger();
    const reg = await registrations();
    expect(await refreshAppTokens({ store, registrations: reg, seeder, logger })).toEqual({ refreshed: ["acme-apps", "beta-apps"], failed: [] });
    ...
    await refreshAppTokens({ store, registrations: reg, seeder, logger });
    expect(written.at(-1)).toEqual({ consumerName: "beta-apps", pat: "ghs_minted_at_tick_2" });
  });

  it("logs the unit whose write fails, with its name, and refreshes the others", async () => {
    const { store } = fakeStore({ value: "ghs_x" });
    const { seeder, written } = fakeSeeder(["acme-apps"]);
    const { logger, errors } = fakeLogger();
    expect(await refreshAppTokens({ store, registrations: await registrations(), seeder, logger })).toEqual({ refreshed: ["beta-apps"], failed: ["acme-apps"] });
  ...
  it("logs, writes nothing and does not reject when the registration tree cannot be read", async () => {
    ...
    expect(await refreshAppTokens({ store, registrations: broken, seeder, logger })).toEqual({ refreshed: [], failed: [] });
  ...
  it("does nothing, and says nothing, on an installation without an App-credentialed unit", async () => {
    ...
    expect(await refreshAppTokens({ store, registrations: reg, seeder, logger })).toEqual({ refreshed: [], failed: [] });
    expect(written).toEqual([]);
    expect(errors).toEqual([]);
