...
const S1_ADDRESS = "203.0.113.10";
const S2_ADDRESS = "203.0.113.20";
...
function seedClusters(dns: FakeDnsProvider): void {
  ...
  dns.seed("s1.example", "A", S1_ADDRESS);
  dns.seed("s2.example", "A", S2_ADDRESS);
}
...
describe("G27 over the tenant's wildcard — validateTenant reads the zone where the plan will write it", () => {
  ...
  it("a leftover of a gone installation passes, names the address it will replace, and asks about the WILDCARD", async () => {
    const asked: string[] = [];
    const outcome = await validateTenant(req(), deps({ standingHost: async (host, fqdn) => { asked.push(`${host} on ${fqdn}`); return { kind: "leftover", standing: "157.90.201.150" }; } }));
    expect(asked).toEqual([`${WILDCARD} on s1.example`]);
    const g27 = outcome.report.gates.find((g) => g.id === "G27");
    expect(g27?.status).toBe("pass");
    expect(g27?.evidence).toEqual([{ source: "manager", name: WILDCARD, fieldPath: "standing", value: "157.90.201.150" }]);
    expect(outcome.verdict).toBe("pass");
  });

  it("a collision with another cluster of this installation fails the verdict, naming that cluster", async () => {
    const outcome = await validateTenant(req(), deps({ standingHost: async () => ({ kind: "collision", standing: S2_ADDRESS, cluster: "s2.example" }) }));
  ...
  it("does not run for a caller that writes no record (no clusterFqdn: add-app, the post-build re-render)", async () => {
    ...
    const outcome = await validateTenant(noZone, deps({ standingHost: async () => ({ kind: "collision", standing: S2_ADDRESS, cluster: "s2.example" }) }));
    expect(outcome.verdict).toBe("pass");
...
describe("create-tenant plans the zone before the first write", () => {
  it("REFUSES a tenant whose wildcard stands at another cluster of this installation — no bucket, no key, no entry", async () => {
    ...
    seedClusters(dns);
    dns.seed(WILDCARD, "A", S2_ADDRESS); // the same subdomain already served by s2
  ...
  it("provision-dns writes the ONE wildcard at the cluster's own address, replacing a leftover and saying what stood there", async () => {
    ...
    const p = CreateTenantParams.parse({
      ...
      members: testMembers([]), identityProvider: "auth", owner: "team-acme", expectedApps: [], catalogRepoUrl: prt.catalogRepoUrl,
