...
import { listOrganisationIdentities, readOrganisationIdentity, recordPackagesReader, recordRepositoryPat, forgetOrganisationCredential, type OrganisationDeps } from "./organisations.ts";
import { registerOrganisationRoutes } from "./api-organisations.ts";
import type { AppEnv } from "../../http/app-env.ts";
import type { OrganisationsListView } from "../../../shared/api-types-organisations.ts";

// The identity of an organisation (#219): a token is MEASURED against GitHub before it is sealed,
...
function deps(over: Partial<OrganisationDeps> = {}): OrganisationDeps {
  return { db: db.db, store, github, githubApp, actor: () => "op_test", ...over };
}

describe("the packages reader of an organisation", () => {
  it("is recorded after the measurement says the token reads the organisation's packages; the list shows its fingerprint and the App's organisation first", async () => {
    ...
    const ids = readOrganisationIdentity(db.db, ORG);
    ...
    const list = await listOrganisationIdentities(deps());
  ...
  it("refuses by name a token that does not read the packages, an invalid one, and an organisation the token cannot see — and seals nothing", async () => {
    ...
    await expect(recordPackagesReader(deps(), "other-org", "ghp_reads")).rejects.toThrow(/knows no organisation "other-org"/);
    github.tokenInvalid = true;
    await expect(recordPackagesReader(deps(), ORG, "ghp_reads")).rejects.toThrow(/invalid or expired/);
    await expect(recordPackagesReader(deps(), "not an org!", "ghp_reads")).rejects.toThrow(/is not a GitHub organisation login/);
    expect(await store.list({ kind: "pat" })).toEqual([]);
    expect(readOrganisationIdentity(db.db, ORG)).toBeNull();
  });

  it("replacing revokes the row that stood; forgetting revokes and clears, and the organisation's row goes with its last credential", async () => {
    await recordPackagesReader(deps(), ORG, "ghp_reads");
    const first = readOrganisationIdentity(db.db, ORG)!.packagesCredentialId!;
    github.orgPackageReaders.set(ORG, ["ghp_reads", "ghp_reads2"]);
    await recordPackagesReader(deps(), ORG, "ghp_reads2");
    const second = readOrganisationIdentity(db.db, ORG)!.packagesCredentialId!;
    expect(second).not.toBe(first);
    await expect(store.open(first, { purpose: "test" })).rejects.toThrow(/revoked/);
    await forgetOrganisationCredential(deps(), ORG, "packages-reader");
    expect(readOrganisationIdentity(db.db, ORG)).toBeNull();
    await expect(store.open(second, { purpose: "test" })).rejects.toThrow(/revoked/);
    await expect(forgetOrganisationCredential(deps(), ORG, "packages-reader")).rejects.toThrow(/records no packages-reader/);
  });
});

describe("the repository PAT of an organisation", () => {
  it("is recorded after its classic scopes are measured (repo + workflow + admin:repo_hook, read:packages not asked), refused fine-grained or short", async () => {
    ...
    expect(readOrganisationIdentity(db.db, "other-org")?.repoCredentialId).toMatch(/^cred_/);
    github.tokenScopes = { classic: true, scopes: ["repo"] };
...
describe("the organisations over HTTP", () => {
  ...
  async function serve(): Promise<{ app: Hono<AppEnv>; cookie: string }> {
    ...
    const app = createApp({
      ...
      registerProtected: (a) => registerOrganisationRoutes(a, deps()),
    });
  ...
  it("PUT measures and records, answering the fingerprint only; GET lists; DELETE forgets; a body without the token is refused by field name", async () => {
    ...
    const put = await app.request(`/api/organisations/${ORG}/packages-reader`, { method: "PUT", ...headers(cookie, { token: "ghp_reads" }) });
    expect(put.status).toBe(200);
    const body = (await put.json()) as { packagesReader: { fingerprint: string } };
    expect(JSON.stringify(body)).not.toContain("ghp_reads");
    const list = (await (await app.request("/api/organisations", headers(cookie))).json()) as OrganisationsListView;
    expect(list.organisations[0]?.packagesReader?.fingerprint).toBe(body.packagesReader.fingerprint);
    const refused = await app.request(`/api/organisations/${ORG}/packages-reader`, { method: "PUT", ...headers(cookie, { pat: "x" }) });
    expect(refused.status).toBe(400);
    expect(await refused.text()).toContain("one field, token");
    const gone = await app.request(`/api/organisations/${ORG}/packages-reader`, { method: "DELETE", ...headers(cookie) });
