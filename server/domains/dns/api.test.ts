...
describe("GET /api/dns", () => {
  ...
  it("answers the book to a signed-in operator, each row judged at the provider, and refuses an anonymous request", async () => {
    ...
    recordDnsWrite(db, { name: "post.example.net", type: "A", content: "203.0.113.9", act: "inserted", owner: { kind: "consumer", name: "post", stage: "prod" }, runId: "run_1" });
    recordDnsWrite(db, { name: "*.acme.example.net", type: "A", content: "203.0.113.9", act: "updated", owner: { kind: "tenant", name: "zsjs023ctne0", stage: "prod" }, runId: "run_2" });
    ...
    dns.seed("*.acme.example.net", "A", "198.51.100.4"); // changed by a hand at the provider since the write
    ...
    expect(body.rows.map((r) => `${r.act} ${r.type} ${r.name} (${r.owner.kind} ${r.owner.name}${r.owner.stage ? " " + r.owner.stage : ""}) by ${r.runId}: ${r.verdict}`).sort()).toEqual([
      "inserted A post.example.net (consumer post prod) by run_1: standing",
      "inserted TXT example.com (mail example.com) by run_3: standing",
      "updated A *.acme.example.net (tenant zsjs023ctne0 prod) by run_2: other",
    ]);
    const wildcard = body.rows.find((r) => r.name === "*.acme.example.net")!;
    expect(wildcard.found).toBe("198.51.100.4");
