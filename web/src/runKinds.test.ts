...
describe("TENANT_RUN_KINDS", () => {
  it("is exactly the tenant lifecycle kinds (create + add/remove-app + the apps repository + suspend/resume/offboard + purge + backup/restore/migrate + the administrator check)", () => {
    expect([...TENANT_RUN_KINDS].sort()).toEqual(["tenant-add-app", "tenant-apps-repo", "tenant-backup", "tenant-check", "tenant-create", "tenant-migrate", "tenant-offboard", "tenant-purge", "tenant-remove-app", "tenant-restart-workloads", "tenant-restore", "tenant-resume", "tenant-set-size", "tenant-suspend"]);
  });
